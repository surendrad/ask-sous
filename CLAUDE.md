  ...
  frontend/
    src/
      ...
      components/
        ...
        RestaurantSwitcher.tsx  # native <select>, not a headless dropdown — see CLAUDE.md note below
      ...
      lib/
        api.ts                # streamChat() (SSE-over-fetch), getRestaurants(), generateCampaign()
        restaurant-context.tsx  # shared selected-restaurant state (React Context)
    tests/
    package.json
  docker-compose.yml
  .env.example
...
- **Access point (Phase 6 — breaking change from the standard envelope):** `POST /chat` (`backend/app/api/chat.py`) — request `{restaurant_id, question}`, response is a **Server-Sent-Events stream** (`text/event-stream`), not the standard `{data, error}` JSON envelope every other endpoint uses. Each event is `data: {...}\n\n` with a `type` discriminator: `text_chunk` (`{text}`), `done` (the old `{answer, tool_calls, model}` shape), or `error` (`{message, code}`, sent for a failure that happens *after* streaming has already started — a failure *before* the first chunk still returns a normal 503/502 JSON response through the usual exception handlers). Calls `answer_question_stream()`, not `answer_question()`. See `docs/decisions/011-sse-streaming-and-mid-stream-errors.md` for the full rationale, including why `curl -d ...` against this endpoint now returns raw SSE frames instead of one JSON body. Checks restaurant existence via the shared `restaurant_exists()` helper (`app/agent/tools/restaurant_lookup.py`) first, as a normal JSON 404 (no SSE stream opened just to error); never imports `llm_client` directly.
...
- **`GET /dashboard?restaurant_id=...`** (Phase 7 — `backend/app/api/dashboard.py`) — response `{kpis, revenue_trend, top_items}`. Not an agent path at all: calls Phase 2's `get_revenue_summary()` and `get_item_velocity()` directly (both already go through `readonly_connection()` internally), never touches `tool_registry.py`/`insights.py`. `top_items` is `get_item_velocity()`'s result re-sorted by `total_quantity` descending — a one-line re-sort, not new aggregation SQL, since that tool's own top-N selection is trend-strength-based rather than volume-based. KPI/revenue values are exact Decimal strings; the frontend formats them to 2 decimal places at render time (`DashboardPage.tsx`'s `formatCurrency()`), not the backend.
...
- Client/UI-only state (selected restaurant, panel toggles): React Context or local `useState` — no need for a separate state library at this scale. Concretely (Phase 6): `RestaurantProvider`/`useRestaurantContext()` (`lib/restaurant-context.tsx`) holds the selected restaurant; `ChatPage`/`CampaignsPanel` are mounted with `key={selectedRestaurant.id}` in `App.tsx` so switching restaurants remounts them and resets their per-restaurant local state (chat history, campaign draft) — a real bug caught during manual verification, not a defensive guess.
- `RestaurantSwitcher.tsx` uses a styled native `<select>` rather than a headless dropdown-menu primitive (design-guidelines.md §8's general pattern) — a deliberate simplification: fully accessible/keyboard-native out of the box, far more reliable to test than a portal-based popup, and this is purely a data-context switch (design-guidelines.md §5) rather than a case where the richer visual treatment carries real product value.
